...
// 필터
const filters = document.querySelector(".item_list_wrap");
if (filters) {
  const filterSwiper = new Swiper(".filters_slider", {
    spaceBetween: 10,
    slidesPerView: "auto",
  });

  const filterBtns = document.querySelectorAll(".filter_slide");
  filterBtns.forEach((filterBtn, idx) => {
    filterBtn.addEventListener("click", (e) => {
      filterBtns.forEach((btn) => {
        btn.classList.remove("active");
      });
      if (Number(filterBtn.dataset.filter) === idx) {
...
if (productList) {
  ...
  tl.from(".product_video", {
    ...
    .from(".product_items, .product_list_wrap", {
      opacity: 0,
    });
}
